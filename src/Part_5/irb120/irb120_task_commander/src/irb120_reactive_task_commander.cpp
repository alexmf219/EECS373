...
// * * * * * * * * * * * * * * * * * * * *
    ...
    //convert path to trajectory and publish
    arrival_time = 4.0;
    pCartTrajPlanner->path_to_traj(optimal_path, arrival_time, new_trajectory);
    ...
    //Begin first movement towards goal location (0.2, 0.4, 0.0)
    
    //Indicates quadrant position of gear with respect to goal (where goal is treated as the origin)
    int indicator = 0;
    
    //Gear is in quadrant 3
    ...
    //extract last joint-space pose from plan to use for start of next plan
    ...
    //First movement adjusts the x coordinate of gear to move it towards the goal
    arrival_time = 2.0;
    ...
    //Begin second movement towards goal location (0.2, 0.4, 0.0)
    //Second movement adjusts the y coordinate of gear to move it towards the goal
    arrival_time = 0.5;
    ...
    //Gear has reached goal location (0.2,0.4,0.0)
    //Gear is in quadrant 3
    arrival_time = 4.0;
    ...
    traj_publisher.publish(new_trajectory); 
    ros::Duration(arrival_time).sleep(); 
    ROS_INFO("done with final trajectory, gear has been moved to goal");
